...
export class ReadWriteIos extends Promises {
    ...
    appendIo(channel: TcaChannel, type: IO_TYPES, timeout: number | undefined = undefined, callback: any = undefined) {
        ...
        // if this IO already exists, clean up
        if (this.queue[id.toString()] !== undefined) {
            this.rejectPromise(id.toString(), "ReadWrite IO queue full, recycle id" + id.toString());
        }
    ...
    rejectAllIos = (channel: TcaChannel | undefined = undefined) => {
        if (channel === undefined) {
            this.rejectAllPromises();
        } else {
            // only reject the IOs from this channel
            for (let [ioId, io] of Object.entries(this.queue)) {
                if (io.channel === channel) {
                    this.rejectPromise(ioId, "Rejecting all IOs for channel" + channel.getChannelName());
                }
            }
        }
    };
