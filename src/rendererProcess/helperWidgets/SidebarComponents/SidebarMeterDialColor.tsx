 ...
 */
export class SidebarMeterDialColor extends SidebarComponent {
	...
	_Element = () => {
		return (
			<Collapsible
				rgbColorStr={this.getMainWidget().getText()["dialColor"]}
				updateFromSidebar={(event: any, propertyName: string, propertyValue: number | string | number[] | string[] | boolean | undefined) => {
					this.updateWidget(event, propertyValue);
				}}
				title={"Color"}
				eventName={"dial-color"}
			/>
		);
	};
	...
	updateWidget = (event: any, propertyValue: number | string | number[] | string[] | boolean | undefined) => {
		...
		const oldVal = this.getText()["dialColor"];
		if (newVal === oldVal) {
			return;
		} else {
			this.getText()["dialColor"] = newVal;
		}
