...
export class SidebarActionItems extends SidebarComponent {
	//todo: more types
	_members: (SidebarActionOpenDisplayItem | SidebarActionWritePvItem | SidebarActionOpenWebpageItem | SidebarActionExecuteCommandItem)[] = [];
	_forceUpdate: any;
	...
	_Element = () => {
		...
		const [createAction, setCreateAction] = React.useState<"OpenDisplay" | "ExecuteCommand" | "OpenWebpage" | "WritePV">("OpenDisplay");
	...
	updateWidgetAppendItem = (event: any, type: "OpenDisplay" | "WritePV" | "OpenWebpage" | "ExecuteCommand") => {
		if (event) {
			event.preventDefault();
		}
		const mainWidget = this.getMainWidget() as ActionButton;
