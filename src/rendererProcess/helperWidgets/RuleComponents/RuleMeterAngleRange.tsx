...
export class RuleMeterAngleRange extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["angleRange"];
	};

	ElementPropertyValue = () => {
        // property value in rule always a string
		const [angleRange, setAngleRange] = React.useState(`${this.getRule().getPropertyValue()}`);

		return (
			<form
				onSubmit={(event: React.FormEvent<HTMLFormElement>) => {
                    this.updatePropertyValue(event, angleRange);
				}}
			...
			>
				...
				<input
					...
					onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
						setAngleRange(event.target.value);
					}}
					// must use enter to change the value
					onBlur={(event: any) => {
						if (`${this.getRuleTdl()["propertyValue"]}` !== `${angleRange}`) {
							setAngleRange(`${this.getRuleTdl()["propertyValue"]}`);
						}
					}}
				/>
			</form>
	...
	evaluatePropertyValue = (input: string): { text: { angleRange?: number } } => {
		try {
