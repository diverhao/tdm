...
export class RuleStepSize extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["stepSize"];
	};

	ElementPropertyValue = () => {
        // property value in rule always a string
		const [stepSize, setStepSize] = React.useState(`${this.getRule().getPropertyValue()}`);

		return (
			<form
				onSubmit={(event: React.FormEvent<HTMLFormElement>) => {
                    this.updatePropertyValue(event, stepSize);
				}}
			...
			>
				...
				<input
					...
					onBlur={(event: any) => {
						if (`${this.getRuleTdl()["propertyValue"]}` !== `${stepSize}`) {
							setStepSize(`${this.getRuleTdl()["propertyValue"]}`);
						}
					}}
				/>
			</form>
	...
	evaluatePropertyValue = (input: string): { text: { stepSize?: number } } => {
		try {
