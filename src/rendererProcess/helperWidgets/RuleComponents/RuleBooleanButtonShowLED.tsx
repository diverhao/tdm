...
export class RuleBooleanButtonShowLED extends RuleComponent {
	...
	ElementPropertyValue = () => {
		// "true" or "false"
		const [showLED, setShowLED] = React.useState(`${this.getRule().getPropertyValue()}`);
		...
		return (
			<form style={this.getRule().getFormStyle()}>
				...
				<input
					type="checkbox"
					checked={mathjs.evaluate(showLED)}
					onChange={(event: any) => {
						if (showLED === "true") {
	...
	evaluatePropertyValue = (input: string): { text: { showLED?: boolean } } => {
		try {
