...
export class RuleMeterPointerThickness extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["pointerThickness"];
	};

	ElementPropertyValue = () => {
        // property value in rule always a string
		const [pointerThickness, setPointerThickness] = React.useState(`${this.getRule().getPropertyValue()}`);

		return (
			<form
				onSubmit={(event: React.FormEvent<HTMLFormElement>) => {
                    this.updatePropertyValue(event, pointerThickness);
				}}
			...
			>
				...
				<input
					...
					onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
						setPointerThickness(event.target.value);
					}}
					// must use enter to change the value
					onBlur={(event: any) => {
						if (`${this.getRuleTdl()["propertyValue"]}` !== `${pointerThickness}`) {
							setPointerThickness(`${this.getRuleTdl()["propertyValue"]}`);
						}
					}}
				/>
			</form>
	...
	evaluatePropertyValue = (input: string): { text: { pointerThickness?: number } } => {
		try {
