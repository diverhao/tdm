...
export class RulePolylineSmootherize extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["smootherize"];
	};

	ElementPropertyValue = () => {
		// "true" or "false"
		const [smootherize, setSmootherize] = React.useState(`${this.getRule().getPropertyValue()}`);
		...
		return (
			<form style={this.getRule().getFormStyle()}>
				...
				<input
					type="checkbox"
					checked={mathjs.evaluate(smootherize)}
					onChange={(event: any) => {
						if (smootherize === "true") {
							this.updatePropertyValue(event, "false");
							setSmootherize("false");
						} else {
	...
	evaluatePropertyValue = (input: string): { text: { smootherize?: boolean } } => {
		try {
