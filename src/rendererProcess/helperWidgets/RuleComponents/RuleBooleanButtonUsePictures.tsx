...
export class RuleBooleanButtonUsePictures extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["usePictures"];
	};

	ElementPropertyValue = () => {
		// "true" or "false"
		const [usePictures, setUsePictures] = React.useState(`${this.getRule().getPropertyValue()}`);
		...
		return (
			<form style={this.getRule().getFormStyle()}>
				...
				<input
					type="checkbox"
					checked={mathjs.evaluate(usePictures)}
					onChange={(event: any) => {
						if (usePictures === "true") {
							this.updatePropertyValue(event, "false");
							setUsePictures("false");
						} else {
	...
	evaluatePropertyValue = (input: string): { text: { usePictures?: boolean } } => {
		try {
