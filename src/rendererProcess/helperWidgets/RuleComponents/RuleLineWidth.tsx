...
export class RuleLineWidth extends RuleComponent {
	...
	ElementPropertyValue = () => {
		...
		const [lineWidth, setLineWidth] = React.useState(`${this.getRule().getPropertyValue()}`);

		return (
			<form
				onSubmit={(event: React.FormEvent<HTMLFormElement>) => {
                    this.updatePropertyValue(event, lineWidth);
				}}
			...
			>
				...
				<input
					...
					onBlur={(event: any) => {
						if (`${this.getRuleTdl()["propertyValue"]}` !== `${lineWidth}`) {
							setLineWidth(`${this.getRuleTdl()["propertyValue"]}`);
						}
					}}
				/>
			</form>
	...
	evaluatePropertyValue = (input: string): { text: { lineWidth?: number } } => {
		try {
