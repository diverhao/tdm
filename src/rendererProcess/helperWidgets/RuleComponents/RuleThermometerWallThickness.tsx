...
export class RuleThermometerWallThickness extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["wallThickness"];
	};

	ElementPropertyValue = () => {
        // property value in rule always a string
		const [wallThickness, setWallThickness] = React.useState(`${this.getRule().getPropertyValue()}`);

		return (
			<form
				onSubmit={(event: React.FormEvent<HTMLFormElement>) => {
                    this.updatePropertyValue(event, wallThickness);
				}}
			...
			>
				...
				<input
					...
					onBlur={(event: any) => {
						if (`${this.getRuleTdl()["propertyValue"]}` !== `${wallThickness}`) {
							setWallThickness(`${this.getRuleTdl()["propertyValue"]}`);
						}
					}}
				/>
			</form>
	...
	evaluatePropertyValue = (input: string): { text: { wallThickness?: number } } => {
		try {
