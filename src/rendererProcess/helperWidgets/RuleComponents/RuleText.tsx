...
export class RuleText extends RuleComponent {
	...
	ElementPropertyValue = () => {
		...
		const [text, setText] = React.useState(`${this.getRule().getPropertyValue()}`);

		return (
			<form
				onSubmit={(event: React.FormEvent<HTMLFormElement>) => {
					this.updatePropertyValue(event, text);
				}}
			...
			>
				...
				<input
					...
					onBlur={(event: any) => {
						if (`${this.getRuleTdl()["propertyValue"]}` !== `${text}`) {
							setText(`${this.getRuleTdl()["propertyValue"]}`);
						}
					}}
				/>
			</form>
	...
	evaluatePropertyValue = (input: string): { text: { text?: string } } => {
		try {
			const result = `${input}`;
