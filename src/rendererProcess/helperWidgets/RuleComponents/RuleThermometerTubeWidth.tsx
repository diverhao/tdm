...
export class RuleThermometerTubeWidth extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["tubeWidth"];
	};

	ElementPropertyValue = () => {
        // property value in rule always a string
		const [tubeWidth, setTubeWidth] = React.useState(`${this.getRule().getPropertyValue()}`);

		return (
			<form
				onSubmit={(event: React.FormEvent<HTMLFormElement>) => {
                    this.updatePropertyValue(event, tubeWidth);
				}}
			...
			>
				...
				<input
					...
					onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
						setTubeWidth(event.target.value);
					}}
					// must use enter to change the value
					onBlur={(event: any) => {
						if (`${this.getRuleTdl()["propertyValue"]}` !== `${tubeWidth}`) {
							setTubeWidth(`${this.getRuleTdl()["propertyValue"]}`);
						}
					}}
				/>
			</form>
	...
	evaluatePropertyValue = (input: string): { text: { tubeWidth?: number } } => {
		try {
