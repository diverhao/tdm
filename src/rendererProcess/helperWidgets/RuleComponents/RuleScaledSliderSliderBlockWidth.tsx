...
export class RuleSliderBlockWidth extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["sliderBlockWidth"];
	};

	ElementPropertyValue = () => {
        // property value in rule always a string
		const [sliderBlockWidth, setSliderBlockWidth] = React.useState(`${this.getRule().getPropertyValue()}`);

		return (
			<form
				onSubmit={(event: React.FormEvent<HTMLFormElement>) => {
                    this.updatePropertyValue(event, sliderBlockWidth);
				}}
			...
			>
				...
				<input
					...
					type="text"
					name="size"
					value={sliderBlockWidth}
					onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
						setSliderBlockWidth(event.target.value);
					}}
					// must use enter to change the value
					onBlur={(event: any) => {
						if (`${this.getRuleTdl()["propertyValue"]}` !== `${sliderBlockWidth}`) {
							setSliderBlockWidth(`${this.getRuleTdl()["propertyValue"]}`);
						}
					}}
				/>
			</form>
	...
	evaluatePropertyValue = (input: string): { text: { sliderBlockWidth?: number } } => {
		try {
