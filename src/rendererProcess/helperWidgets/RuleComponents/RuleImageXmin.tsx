...
export class RuleImageXmin extends RuleComponent {
	...
	ElementPropertyValue = () => {
		const [xmin, setXmin] = React.useState(`${this.getRule().getPropertyValue()}`);

		return (
			<form
				onSubmit={(event: React.FormEvent<HTMLFormElement>) => {
                    this.updatePropertyValue(event, xmin);
                }}
			...
			>
				...
				<input
					...
					onBlur={(event: any) => {
						if (`${this.getRuleTdl()["propertyValue"]}` !== `${xmin}`) {
							setXmin(`${this.getRuleTdl()["propertyValue"]}`);
						}
					}}
				/>
			</form>
