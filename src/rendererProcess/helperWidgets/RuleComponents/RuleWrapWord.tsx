...
export class RuleWrapWord extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["wrapWord"];
	};

	ElementPropertyValue = () => {
		// "true" or "false"
		const [wrapWord, setWrapWord] = React.useState(`${this.getRule().getPropertyValue()}`);
		...
		return (
			<form style={this.getRule().getFormStyle()}>
				...
				<input
					type="checkbox"
					checked={mathjs.evaluate(wrapWord)}
					onChange={(event: any) => {
						if (wrapWord === "true") {
	...
	evaluatePropertyValue = (input: string): { text: { wrapWord?: boolean } } => {
		try {
