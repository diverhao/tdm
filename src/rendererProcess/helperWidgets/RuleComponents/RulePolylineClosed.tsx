...
export class RulePolylineClosed extends RuleComponent {
	...
	ElementPropertyValue = () => {
		// "true" or "false"
		const [closed, setClosed] = React.useState(`${this.getRule().getPropertyValue()}`);
		...
		return (
			<form style={this.getRule().getFormStyle()}>
				...
				<input
					type="checkbox"
					checked={mathjs.evaluate(closed)}
					onChange={(event: any) => {
						if (closed === "true") {
	...
	evaluatePropertyValue = (input: string): { text: { closed?: boolean } } => {
		try {
