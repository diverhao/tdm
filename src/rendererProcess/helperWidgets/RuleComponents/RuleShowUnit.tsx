...
export class RuleShowUnit extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["showUnit"];
	};

	ElementPropertyValue = () => {
		// "true" or "false"
		const [showUnit, setShowUnit] = React.useState(`${this.getRule().getPropertyValue()}`);
		...
		return (
			<form style={this.getRule().getFormStyle()}>
				...
				<input
					type="checkbox"
					checked={mathjs.evaluate(showUnit)}
					onChange={(event: any) => {
						if (showUnit === "true") {
	...
	evaluatePropertyValue = (input: string): { text: { showUnit?: boolean } } => {
		try {
