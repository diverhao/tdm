...
export class RuleUseLogScale extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["useLogScale"];
	};

	ElementPropertyValue = () => {
		// "true" or "false"
		const [useLogScale, setUseLogScale] = React.useState(`${this.getRule().getPropertyValue()}`);
		...
		return (
			<form style={this.getRule().getFormStyle()}>
				...
				<input
					type="checkbox"
					checked={mathjs.evaluate(useLogScale)}
					onChange={(event: any) => {
						if (useLogScale === "true") {
	...
	evaluatePropertyValue = (input: string): { text: { useLogScale?: boolean } } => {
		try {
