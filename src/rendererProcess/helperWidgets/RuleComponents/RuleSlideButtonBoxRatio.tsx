...
export class RuleSlideButtonBoxRatio extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["boxRatio"];
	};

	ElementPropertyValue = () => {
        // property value in rule always a string
		const [boxRatio, setBoxRatio] = React.useState(`${this.getRule().getPropertyValue()}`);

		return (
			<form
				onSubmit={(event: React.FormEvent<HTMLFormElement>) => {
                    this.updatePropertyValue(event, boxRatio);
				}}
			...
			>
				...
				<input
					...
					onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
						setBoxRatio(event.target.value);
					}}
					// must use enter to change the value
					onBlur={(event: any) => {
						if (`${this.getRuleTdl()["propertyValue"]}` !== `${boxRatio}`) {
							setBoxRatio(`${this.getRuleTdl()["propertyValue"]}`);
						}
					}}
				/>
			</form>
	...
	evaluatePropertyValue = (input: string): { text: { boxRatio?: number } } => {
		try {
