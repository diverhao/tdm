...
export class RuleShowPvValue extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["showPvValue"];
	};

	ElementPropertyValue = () => {
		// "true" or "false"
		const [showPvValue, setShowPvValue] = React.useState(`${this.getRule().getPropertyValue()}`);
		...
		return (
			<form style={this.getRule().getFormStyle()}>
				...
				<input
					type="checkbox"
					checked={mathjs.evaluate(showPvValue)}
					onChange={(event: any) => {
						if (showPvValue === "true") {
							this.updatePropertyValue(event, "false");
							setShowPvValue("false");
						} else {
	...
	evaluatePropertyValue = (input: string): { text: { showPvValue?: boolean } } => {
		try {
