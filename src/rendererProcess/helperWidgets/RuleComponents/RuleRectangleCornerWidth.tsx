...
export class RuleRectangleCornerWidth extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["cornerWidth"];
	};

	ElementPropertyValue = () => {
        // property value in rule always a string
		const [cornerWidth, setCornerWidth] = React.useState(`${this.getRule().getPropertyValue()}`);

		return (
			<form
				onSubmit={(event: React.FormEvent<HTMLFormElement>) => {
                    this.updatePropertyValue(event, cornerWidth);
				}}
			...
			>
				...
				<input
					...
					onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
						setCornerWidth(event.target.value);
					}}
					// must use enter to change the value
					onBlur={(event: any) => {
						if (`${this.getRuleTdl()["propertyValue"]}` !== `${cornerWidth}`) {
							setCornerWidth(`${this.getRuleTdl()["propertyValue"]}`);
						}
					}}
				/>
			</form>
	...
	evaluatePropertyValue = (input: string): { text: { cornerWidth?: number } } => {
		try {
