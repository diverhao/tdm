...
export class RuleByteMonitorBitLength extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["bitLength"];
	};

	ElementPropertyValue = () => {
        // property value in rule always a string
		const [bitLength, setBitLength] = React.useState(`${this.getRule().getPropertyValue()}`);

		return (
			<form
				onSubmit={(event: React.FormEvent<HTMLFormElement>) => {
                    this.updatePropertyValue(event, bitLength);
				}}
			...
			>
				...
				<input
					...
					onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
						setBitLength(event.target.value);
					}}
					// must use enter to change the value
					onBlur={(event: any) => {
						if (`${this.getRuleTdl()["propertyValue"]}` !== `${bitLength}`) {
							setBitLength(`${this.getRuleTdl()["propertyValue"]}`);
						}
					}}
				/>
			</form>
	...
	evaluatePropertyValue = (input: string): { text: { bitLength?: number } } => {
		try {
