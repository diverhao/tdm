...
export class RuleArcAngleStart extends RuleComponent {
	...
	getWidgetValue = () => {
		...
		return text["angleStart"];
	};

	ElementPropertyValue = () => {
        // property value in rule always a string
		const [angleStart, setAngleStart] = React.useState(`${this.getRule().getPropertyValue()}`);

		return (
			<form
				onSubmit={(event: React.FormEvent<HTMLFormElement>) => {
                    this.updatePropertyValue(event, angleStart);
				}}
			...
			>
				...
				<input
					...
					onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
						setAngleStart(event.target.value);
					}}
					// must use enter to change the value
					onBlur={(event: any) => {
						if (`${this.getRuleTdl()["propertyValue"]}` !== `${angleStart}`) {
							setAngleStart(`${this.getRuleTdl()["propertyValue"]}`);
						}
					}}
				/>
			</form>
	...
	evaluatePropertyValue = (input: string): { text: { angleStart?: number } } => {
		try {
