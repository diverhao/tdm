...
export class VideoRecorder {
	...
	save = async (blobs: Blob[], folder: string) => {
        const videoFileName = path.join(folder,`${this.getDisplayWindowClient().getWindowId()}-${Date.now()}.webm`);
		if (blobs.length >= 1) {
			const a = await blobs[0].arrayBuffer();
			const buf = Buffer.from(a);

            fs.writeFile(videoFileName, buf, function (err) {
				if (err) {
					console.error("Failed to save video " + err);
				} else {
					Log.info("Saved video: " + videoFileName);
				}
			});
		} else {
            Log.info("No data");
