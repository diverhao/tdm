...
export class ArcHelper extends BaseWidgetHelper {
	...
	static convertBobToTdl = (bob: Record<string, any>, type: "ellipse" | "arc"): type_Arc_tdl => {
		console.log("\n------------", `Parsing ${type}`, "------------------\n");
		...
		for (const propertyName of propertyNames) {
			...
			if (propertyValue === undefined) {
				...
				if (propertyName === "x") {
					...
					tdl["text"]["angleStart"] = parseInt(propertyValue);
				} else if (propertyName === "total_angle") {
					tdl["text"]["angleRange"] = parseInt(propertyValue);
