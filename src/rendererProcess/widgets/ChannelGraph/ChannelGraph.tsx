...
export class ChannelGraph extends BaseWidget {
	...
	_ElementChannelGraph = () => {
		...
		return (
			<>
				<this._ElementChannelGraphBox top={50} left={100} width={200} height={200}></this._ElementChannelGraphBox>
				<this._ElementChannelGraphBox top={250} left={500} width={200} height={200}></this._ElementChannelGraphBox>
			</>
		);  
	};

	_ElementChannelGraphBox = ({ top, left, width, height }: any) => {
		return (
			<div
				style={{
					display: "inline-flex",
			...
			>
				<svg viewBox="0 0 100 100" overflow={"visible"}>
					<circle cx="50" cy="50" r="50" fill="none" stroke="blue" strokeWidth="5px"></circle>
				</svg>
			</div>
		);
	};

	_ElementChannelGraphLine = ({ point1X, point1Y, point2X, point2Y }: any) => {};
