import { ThumbWheel, type_ThumbWheel_tdl } from "./ThumbWheel";
import { BaseWidgetRules } from "../BaseWidget/BaseWidgetRules";
import {ThumbWheelRule } from "./ThumbWheelRule"


export class ThumbWheelRules extends BaseWidgetRules {
	constructor(mainWidget: ThumbWheel, widgetTdl: type_ThumbWheel_tdl) {
		super(mainWidget, widgetTdl);
        this.initRules();
	}

    
	initRules = () => {
		for (let ii = 0; ii < this.getRulesTdl().length; ii++) {
            const newRule = new ThumbWheelRule(ii, this);
			this.getRules().push(newRule);
		}
	};

    // (1) create a new tdl
    // (2) create the new rule based on this new tdl, use the widget's value
	addRule = () => {
        // (1)
		const newRuleTdl = this.generateNewRuleTdl();
		this.getRulesTdl().push(newRuleTdl);
        // (2)
		const newRule = new ThumbWheelRule(this.getRulesTdl().length - 1, this);
        newRule.renewRuleComponent(true);
		this.getRules().push(newRule);
	};
}
