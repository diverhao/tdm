...
skipStatement
  = code:$((!("state" / "ss" / "when" / "entry" / "exit" / "{" / "}") .)+ _ EOF?) {
      return { type: "skip", value: code };
  }
...
export const parseSeq = (input: string, emulationMode: boolean) => {
    ...
    const grammarUsed = emulationMode ? grammar_emulation : grammar;
    const parser = peg.generate(grammarUsed);
